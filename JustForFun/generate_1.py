...
def get_data_unternehmenIndex(cursor, i_id):  # get data from each unternehmen in certain indey
    cursor.execute(
...
def insert_data(cursor, u_id, gen):
    # Erstellen Sie hier Ihre INSERT-Anweisung. Zum Beispiel:
    query = f"INSERT INTO kursdaten(u_id, aktienWert) VALUES ({u_id}, {gen})"
    print(query)
...
def main(config):
    ...
    try:
        while True:  # Das sorgt für eine Endlosschleife
            # Daten abrufen
            data = get_data_Index(cursor)  # get all index
            print(data)
